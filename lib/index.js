  ...
  return {
    genHeader() {
      ...
      doc
        .fontSize(16)
        .fillColor("#cccccc")
        .text(moment().format("MMMM, DD, YYYY"), CONTENT_LEFT_PADDING, 45, {
          align: "right"
        })
        .fillColor("#333333");
    ...
    genFooter() {
      ...
      doc.fontSize(12).text(company.name, CONTENT_LEFT_PADDING, 650);
    ...
    genTableHeaders() {
      ["amount", "quantity", "description"].forEach((text, i) => {
        console.log(`${table.x + i * table.inc} x ${table.y}`);
        doc
          .fontSize(TEXT_SIZE)
